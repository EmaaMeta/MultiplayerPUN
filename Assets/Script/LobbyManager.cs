...
public class LobbyManager : MonoBehaviourPunCallbacks // if we need to do something once connect will change it to call back see below override 
{
    ...
    { 
        // 1. delete all item and clear list
        ...
        //2. spawn item for each player in the room and add it to the list
        if (PhotonNetwork.CurrentRoom == null) // to check if we are in room
        {
            return; // Exit
        }
        foreach (KeyValuePair<int, Player> player in PhotonNetwork.CurrentRoom.Players) // loop to the number of player in the room
        {
            PlayerItem newPLayerItem = Instantiate(playerItemPrefab, playerItemParent);
            newPLayerItem.SetPlayerInfo(player.Value);

            if (player.Value == PhotonNetwork.LocalPlayer) // to update only local player
            { 
